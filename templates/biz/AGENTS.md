...
> biz 프리셋은 코드 구현(worker) 대신 조사·문서(researcher)를 상시 투입하는 운영 중심 체제입니다:
> orchestrator(오케스트레이션) → planner(과제 정의) → researcher(공고·정책·사례 조사) → reviewer(요건·마감·수치 검증).
> 행정·정부지원사업, CS 매뉴얼, 운영 자동화가 주된 산출물이며, 구현/자동화가 필요하면 PM 승인으로 `{{PREFIX}}-worker`를 on-demand 기동합니다.
...
[ Orchestrator ({{PREFIX}}-orchestrator) ] ── 파이프라인 실시간 중계 & 드라이브 (코드 수정 금지)
   │
   ├─ 1) 과제 정의 지시 ──► [ {{PREFIX}}-planner ] 과제 정의·자격요건 체크리스트 + 조사 항목 분할
   │        │                    │
   │        ▼ 명세 수령        완료 즉시 다음 연결
   ├─ 2) 조사 지시 ──────► [ {{PREFIX}}-researcher ] 공고·정책·사례 조사·비교 + 근거 보고 (코드 수정 금지)
   │        │                    │
   │        ▼ 결과물 전달    완료 즉시 다음 연결
   └─ 3) 검증/리뷰 지시 ──► [ {{PREFIX}}-reviewer ] 요건·마감·수치 검증 ── [APPROVE + 검증 로그] ──► Orchestrator → PM 보고
                                │
