...
(`herdr-team-setup {{PREFIX}}` 실행 시 `{{PREFIX}}`에 실제 프로젝트 prefix가 치환됩니다.)
...
`{{PREFIX}}-qa`는 폐지되었습니다. 실행 기반 테스트 검증은 `{{PREFIX}}-reviewer`가 통합 담당합니다.
...
- **On-Demand 에이전트**: 상시 팀이 아니며 PM이 필요 시에만 기동합니다.
  - `{{PREFIX}}-researcher`: 외부 리서치, 기술 조사, 라이브러리/SDK 내부 탐사 전담 (읽기·보고만, 코드 수정 금지).
...
- **수정 권한**: 코드를 직접 수정하는 것은 `{{PREFIX}}-worker`뿐입니다. `{{PREFIX}}-reviewer`/`{{PREFIX}}-researcher`는 수정하지 않고 보고서만 냅니다 (`{{PREFIX}}-ops`는 승인된 운영 범위만 예외).
...
- 원클릭 셋업: `herdr-team-setup {{PREFIX}}` (상세는 [README](../README.md) 참조).
...
herdr tab create --cwd "$PWD" --label "{{PREFIX}}-team" --no-focus
...
herdr pane rename "$P_PLANNER" "② Planner ({{PREFIX}}-planner)"
herdr pane rename "$P_WORKER" "③ Worker ({{PREFIX}}-worker)"
herdr agent rename {{PREFIX}}-planner "{{PREFIX}}-planner"
herdr agent rename {{PREFIX}}-worker "{{PREFIX}}-worker"
```
